...
public class FixedExtensionServiceImpl implements FixedExtensionService {
	...
	@Override
	public ExtensionInfo add(String extension, FixedExtensionState state) {
		...
		if (fixedExtensionRepository.count() >= 200)
			throw new DuplicateResourceException("이미 등록된 고정 확장자가 200개 이상입니다.");

		return extensionMapper.toInfo(
				fixedExtensionRepository.save(
						FixedExtensionEntity.builder()
						.extension(extension.trim().toLowerCase())
						.state(state).build()));
	}

	@Override
	public ExtensionInfo checkOrUncheck(long extensionNo, FixedExtensionState state) {
	...
	@Override
	public List<ExtensionInfo> search(FixedExtensionState state) {

		List<FixedExtensionEntity> list = null;
		if (state == null) {
			list = fixedExtensionRepository.findAll();
		} else {
			list = fixedExtensionRepository.findByState(state);
		}

		return list.stream().map((entity) -> extensionMapper.toInfo(entity))
